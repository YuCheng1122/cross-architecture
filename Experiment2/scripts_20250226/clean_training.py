...
def process_pcode_operations(pcode_list):
    ...
    for instruction in pcode_list:
        ...
        # Skip lines that start with " --- " and don't try to match operations
        if instruction.startswith(" --- "):
            # Still need to count branching operations
        ...
        # Try different regex patterns to extract operations
        op_match = None
        
        # Pattern 1: Look for operation between parentheses and next open parenthesis
        if not op_match:
            op_match = re.search(r'\)\s+([A-Z_0-9]+)\s+\(', instruction)
        
        # Pattern 2: Look for operation at the beginning of the line (for some formats)
        if not op_match:
            op_match = re.search(r'^([A-Z_0-9]+)\s+\(', instruction)
        
        # If we found an operation, categorize it
        if op_match:
...
def analyze_function_calls(function_addr, function_calls):
    ...
    return {
        "FUNCTION_OUT_DEGREE": out_degree,
        "FUNCTION_IN_DEGREE": in_degree
    }
    ...
    try:
        ...
        for func_addr, pcode_list in pcode_data.items():
            ...
            for metric, value in function_features.items():
                if isinstance(value, (int, float)):  # Ensure only numeric values are aggregated
...
def get_metadata_map():
    ...
    from file name to CPU and label/family information
    """
    metadata_file = "/home/tommy/cross-architecture/Experiment2/csv/Cross-arch Dataset_20250221140826.csv"
    try:
        ...
        for _, row in df.iterrows():
            ...
            metadata_map[clean_name] = {
                'CPU': row.get('CPU', ''),
                'label': row.get('label', ''),  # For benign samples
                'family': row.get('family', '')  # For malware samples
            }
        return metadata_map
    except Exception as e:
...
def save_to_csv(data_list, output_file_prefix, metadata_map, is_malware=False):
    """
    ...
    Args:
        ...
        is_malware: Whether this is malware data (uses 'family') or benign (uses 'label')
    """
    ...
    # Choose the appropriate label field based on data type
    label_field = "family" if is_malware else "label"
    ...
    with open(f"{output_file_prefix}_function_features.csv", 'w', newline='') as f:
        ...
        # Write header with metadata columns first
        header = ["file_name", "CPU", label_field, "function_name"] + list(sorted(feature_keys))
        writer.writerow(header)
        ...
        for data in data_list:
            ...
            for func in data["functions"]:
                ...
                row.extend([func["function_level_features"].get(key, "") for key in header[4:]])
                writer.writerow(row)
...
def process_dataset(dataset_type):
    """
    Process all files in the specified dataset (train or test)
    """
    base_dir = "/home/tommy/cross-architecture/Experiment2/dataset"
    output_dir = "/home/tommy/cross-architecture/Experiment2/csv"
    logs_dir = "/home/tommy/cross-architecture/Experiment2/logs"
    ...
    log_file = os.path.join(logs_dir, f"{current_date}_{dataset_type}_cleaning.log")
    ...
    with open(log_file, 'w') as log:
        log.write(f"Processing {dataset_type} dataset on {datetime.datetime.now()}\n")
        log.write("-" * 80 + "\n")
    
    # Process benign samples
    benign_dir = os.path.join(base_dir, dataset_type, "benign")
    benign_files = glob.glob(os.path.join(benign_dir, "**", "*.json"), recursive=True)
    print(f"Found {len(benign_files)} benign files to process in {dataset_type} dataset")
    
    benign_data = process_files_batch(
        benign_files, 
        log_file, 
        f"Processing benign {dataset_type} files"
    )
    
    # Process malware samples
    malware_dir = os.path.join(base_dir, dataset_type, "malware")
    malware_files = glob.glob(os.path.join(malware_dir, "**", "*.json"), recursive=True)
    print(f"Found {len(malware_files)} malware files to process in {dataset_type} dataset")
    
    malware_data = process_files_batch(
        malware_files, 
        log_file, 
        f"Processing malware {dataset_type} files"
    )
    
    # Save benign data (uses 'label' field)
    benign_output_prefix = os.path.join(output_dir, f"{current_date}_cleaned{dataset_type}_benign")
    save_to_csv(benign_data, benign_output_prefix, metadata_map, is_malware=False)
    print(f"Saved {len(benign_data)} benign samples to {benign_output_prefix}_*.csv")
    
    # Save malware data (uses 'family' field)
    malware_output_prefix = os.path.join(output_dir, f"{current_date}_cleaned{dataset_type}_malware")
    save_to_csv(malware_data, malware_output_prefix, metadata_map, is_malware=True)
    print(f"Saved {len(malware_data)} malware samples to {malware_output_prefix}_*.csv")
    
    # Log completion
    with open(log_file, 'a') as log:
        log.write(f"Completed processing {dataset_type} dataset. Processed {len(benign_data)} benign and {len(malware_data)} malware samples.\n")
