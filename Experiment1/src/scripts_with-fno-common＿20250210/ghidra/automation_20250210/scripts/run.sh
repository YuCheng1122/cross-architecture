...
OLD_OUT_DIR="/home/tommy/cross-architecture/Experiment1/src/scripts_with-g＿20250204/ghidra/automation_20250209/unpackedTuneDB"  # Java 程式輸出結果目錄

DATA_DIR="$(pwd)/data"                                  # 資料存放目錄（包含各 CPU 架構子目錄）
RESULTS_DIR="$(pwd)/results"                            # 分析結果最終存放目錄

# 若 PROJECT_DIR 不存在，則建立之
if [ ! -d "$PROJECT_DIR" ]; then
    echo "建立 PROJECT_DIR: $PROJECT_DIR"
    mkdir -p "$PROJECT_DIR"
fi

# 定義全局 timestamp（所有結果統一放在此子目錄下）
...
for archDir in "$DATA_DIR"/*; do
    if [ -d "$archDir" ]; then
        ...
        for binary in "$archDir"/*; do
            ...
            # 移動結果到 results/TIMESTAMP/架構子目錄，並以原檔名儲存
            DEST_DIR="$RESULTS_DIR/$TIMESTAMP/$arch"
            mkdir -p "$DEST_DIR"
