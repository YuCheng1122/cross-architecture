...
# 自動化相關目錄
AUTOMATION_DIR="$SCRIPT_DATE_DIR/ghidra/automation_20250206"
...
java ComparePcode \
    "$OUTPUT_DIR/arm" \
    "$OUTPUT_DIR/mips" \
    "$COMPARISON_DIR/comparison_results_${TIMESTAMP}.csv" \
    2>&1 | tee -a "$LOG_FILE"

echo "完整分析流程完成！" | tee -a "$LOG_FILE"
echo "結果保存在: $OUTPUT_DIR" | tee -a "$LOG_FILE"
echo "比對結果: $COMPARISON_DIR/comparison_results_${TIMESTAMP}.csv" | tee -a "$LOG_FILE"
